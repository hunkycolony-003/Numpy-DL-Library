...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if self.normalization == "batchnorm":
               out, cache = affine_bn_relu_forward(out, 
                ...
                out, cache = affine_relu_forward(out, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
        ...
        loss, grads = 0.0, {}
        loss, dscores = softmax_loss(scores, y)
        reg_loss = sum(np.sum(self.params[w] * self.params[w]) for w in self.params.keys() if "W" in w)
        ...
        for i in reversed(range(self.num_layers - 1)):
            # dropout gradient, removes dropout cache
            if self.use_dropout:
